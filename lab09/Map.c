...
{
    	assert (g != NULL);
	VList startList = g->connections[start];
	VList node = startList;
	int count = 0;
	if (isLand(start) && isLand(end)) {
		...
		while (node != NULL) {
			...
			if (node->v == end) {
				type[count] = node->type;
				count++;
			}

			//checking for boat connection, from 2 ports
			if (isSea(node->v)) {
				VList seaList = g->connections[node->v];
				while (seaList != NULL) {
					if (seaList->v == end) {
						type[count] = seaList->type;
						count++;
					}
					seaList = seaList->next;
				}
			}
