...
void BSTreeLevelOrder (BSTree t)
{
	...
	Queue myQ = newQueue();
	printf( "%d ",t->value);
	if (t->left != NULL) QueueJoin(myQ, t->left);
	if (t->right != NULL) QueueJoin(myQ, t->right);
	while (!QueueIsEmpty(myQ)){
		Item val = QueueLeave(myQ);
		printf( "%d ",val->value);
		if (val->left != NULL) QueueJoin(myQ, val->left);
		if (val->right != NULL) QueueJoin(myQ, val->right);
	}
	dropQueue(myQ);
}
